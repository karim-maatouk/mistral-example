...
def load_finetuned_model(adapter_config_path, adapter_model_path, model_id='mistralai/Mistral-7B-v0.1', max_tokens=512):
    ...
    if model is None or tokenizer is None:
        ...
        config = PeftConfig.from_pretrained(adapter_config_path)  # ✅ Load config properly

        logger.info('Loading fine-tuned model...')
        model = PeftModel.from_pretrained(base_model, adapter_model_path, config=config).eval()  # ✅ Pass config object
